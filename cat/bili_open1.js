...
async function category(tid, page, filter, ext) {
    ...
    try {
        ...
        if (tid.endsWith('_clicklink')) {
            tid = tid.split('_')[0];
            url = "https://api.bilibili.com//x/space/wbi/arc/search?mid=" + tid + '&order=pubdate&ps=30&pn=' + page;
            const data = JSON.parse(await request(url, getHeaders())).data;
            let items = data.result;
            items = data.list.vlist;
            const videos = [];
            for (const item of items) {
                ...
                let cd = getFullTime(item.length);
...
async function detail(ids) {
    try {
        ...
        const up_name = detailData.owner.name;
        const up_mid = detailData.owner.mid;
        const video = {
            vod_id: bvid,
            ...
            vod_area: '',
            vod_remarks: cd,
            vod_actor: '❤️发布日期：' + Y+M+D+h+m+s,
            vod_director: '🔥UP主：' + '[a=cr:' + JSON.stringify({'id':up_mid + '_clicklink','name':up_name}) + '/]' + up_name + '[/a]',
